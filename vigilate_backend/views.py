...
class UserProgramsViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request):
        ...
        for elem in query['programs_list']:
            prog = UserPrograms.objects.filter(user_id=request.user.id, program_name=elem['program_name'], poste=station)
            ...
            if prog:
                ...
                new_prog = UserPrograms(user_id=request.user, minimum_score=1, poste=station,
            ...
            if only_one_program:
                obj = UserPrograms.objects.get(user_id=request.user.id, program_name=elem['program_name'], poste=station)
